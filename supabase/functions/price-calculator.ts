...
const calculateRouteFromOSM = async (
  ...
  try {
    ...
    throw new Error(
      `Impossible de calculer la route via OpenStreetMap: ${error instanceof Error ? error.message : "Erreur inconnue"}`,
    );
  }
};
...
serve(async (req: Request) => {
  try {
    console.log("[DEBUG] Edge function price-calculator démarrée", {
      method: req.method,
      url: req.url,
    });
    ...
    if (isUpdate) {
      ...
      const optionsChanged =
        Array.isArray(ride.options) && Array.isArray(oldRide.options)
      ...
      if (!coordinatesChanged && !vehicleTypeChanged && !optionsChanged) {
        console.log(
          "[DEBUG] No significant changes detected, skipping recalculation",
        );
        return new Response(
          JSON.stringify({
            success: true,
    ...
    if (Array.isArray(ride.options)) {
      ...
      const validOptionsInPayload = ride.options.filter((opt: string) =>
        validOptions.includes(opt),
      );

      if (validOptionsInPayload.length > 0) {
        const optionsPrice = validOptionsInPayload.reduce(
          (sum: number, opt: string) => {
            switch (opt) {
              case "child_seat":
                return sum + 15;
              case "pet_friendly":
