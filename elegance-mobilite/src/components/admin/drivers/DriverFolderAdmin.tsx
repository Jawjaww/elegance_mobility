...
export default function DriverFolderAdmin({ driverId }: { driverId: string }) {
  ...
  async function loadData() {
    ...
    try {
      ...
      // Générer les signed URLs pour les documents du bucket privé
          ...
          if (doc.file_url && !doc.file_url.startsWith("http")) {
            try {
              const { data: urlData } = await supabase.storage
                ...
                .createSignedUrl(doc.file_url, 3600); // 1h
      ...
      // Vérification automatique via la fonction Supabase
                ...
                {typeDocs.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {typeDocs.map((d) => {
                      ...
                      const isImage = d.file_url?.match(
                      ...
                      return (
                        ...
                        >
                          ...
                          {signedUrl && isImage ? (
                            <img
                              src={signedUrl}
                              alt={d.file_name ?? d.document_type}
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            {d.validation_status === "pending" && (
                              <>
                                ...
                                <Button
                                  size="sm"
                                  onClick={() => validateDoc(d.id, false)}
                                  variant="destructive"
                                  className="text-xs"
                                >
                                  Rejeter
        ...
        {debugDetails.length > 0 && (
          <details className="mb-6">
            ...
            <div className="mt-3 space-y-1 max-h-64 overflow-y-auto">
              {debugDetails.map((d: any, i: number) => (
