...
export function RideCard({
  ...
  const formattedPickupTime = format(new Date(ride.pickup_time), "HH'h'mm", {
    locale: fr,
  });
    ...
    <Card className="overflow-hidden dark:bg-gray-800/60 rounded-lg backdrop-blur-lg">
        ...
        <div className="p-4 grid gap-3">
          ...
          <div className="flex items-center text-sm">
            ...
            <span className="text-gray-200">
              {format(new Date(ride.pickup_time), "EEEE d MMMM", {
                locale: fr,
              })}
            </span>
          </div>
          ...
          {ride.user_id && (
            <div className="flex items-center text-sm">
              ...
              <span className="text-gray-200">
                Client #{ride.user_id.substring(0, 8)}
              </span>
            </div>
          )}
        </div>
      ...
      <CardFooter className="flex justify-between py-3 px-4 border-t border-gray-700 bg-gray-800/40">
        <Button
          ...
          onClick={() => onDetailsClick && onDetailsClick(ride.id)}
        >
          Détails
        </Button>
