...
function capitalizeWord(str: string | undefined): string {
  if (!str) return "";
  return str.charAt(0).toUpperCase() + str.slice(1).toLowerCase();
}

function DetailTripFacts({ ride }: Readonly<{ ride: Reservation }>) {
      ...
      {ride.vehicle_type ? (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-neutral-100">{capitalizeWord(ride.vehicle_type)}</p>
          </div>
        </div>
      ) : null}
  ...
  return (
    <DialogContent className="bg-neutral-900 border-neutral-800 text-neutral-100 sm:max-w-md">
      ...
      <div className="space-y-6 pt-4">
        ...
        <CancelPolicyLine
          text={cancelChipWithBilling(cancelChip, ride.cancel_billing)}
        />
        <DetailTripFacts ride={ride} />
